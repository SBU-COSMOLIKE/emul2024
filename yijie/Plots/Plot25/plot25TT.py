...
pred=np.load('cos_TT_pred.npy',allow_pickle=True)
camb=np.load('cos_TT_true.npy',allow_pickle=True)
...
axs[1].text(1000, 1e-3, '$1/\ell$', fontsize = 22)
...
axs[0].set_ylabel(r'$\ell(\ell+1)C^{\rm TT}_{\ell}/2\pi[\mu K^2]$',fontsize=16)
axs[1].set_ylabel(r'$\Delta C^{\rm TT}_{\ell}/C_{\ell}^{\rm TT,CAMB}$',fontsize=16)
axs[1].set_xlabel(r'$\ell$',fontsize=14)

#plt.setp(axs[0].get_xticklabels(),visible=True)
plt.subplots_adjust(hspace=0)

fig.savefig("TTcomp.pdf", format="pdf", bbox_inches="tight",dpi=300, pad_inches=0.01)