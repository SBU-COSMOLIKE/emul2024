...
def predict(X):
    with torch.no_grad():
        ...
        par=X.float().numpy()
        y_pred=y_pred*rescaling.par_to_rescale(par,camb_ell_min,camb_ell_max)
    return y_pred
...
plt.legend()
plt.savefig("gutcmbres1mil.pdf", format="pdf", bbox_inches="tight")
...
#plt.legend()
plt.savefig("gutcmbreserror1mil.pdf", format="pdf", bbox_inches="tight")
...
#plt.axhline(y = 0, color = 'grey', linestyle = '-') 
plt.savefig("gutcmbreschi1mil.pdf", format="pdf", bbox_inches="tight")