...
d1=np.load('test_pt.npy',allow_pickle=True)[:,:6]
...
g.plots_2d([samples2],param_pairs=[['omb','omc'],['H0', 'ns'], ['H0','logAs']],nx=3,legend_labels=[r'$T_{\rm test}=128$'],filled=[True],colors=['firebrick'],contour_lws=[1.2])
g.subplots[0,0].plot(d1[:,0],d1[:,1],'ko',label='Test Demo')
g.subplots[0,1].plot(d1[:,2],d1[:,4],'ko')
g.subplots[0,2].plot(d1[:,2],d1[:,5],'ko')
#plt.legend(loc='lower left',fontsize=13)
#g.add_legend(legend_labels=[r'$T_{\rm test}=128$','Test Demo'], legend_loc='lower left',fontsize=13)
g.fig.savefig('test128.pdf',format="pdf", bbox_inches="tight", dpi=300, pad_inches=0.05)
"""
