...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
model = MLP(input_dim=9,output_dim=501,int_dim=intdim,N_layer=Nlayer,dropout=0)
model.to(device)
...
model.eval()
cheby_mat=np.load('chebytrans500.npy',allow_pickle=True)
cheby_mat=torch.Tensor(cheby_mat).to(device)
ell=np.arange(camb_ell_min,camb_ell_max,1)
#test_ll = (ell-(camb_ell_max+camb_ell_min)/2)/((camb_ell_range)/2)
ell=torch.Tensor(ell).to(device)
def predict(X):
    with torch.no_grad():
        ...
        y_pred = torch.t(cheby_mat@torch.t(y_pred)/ell[:,None]/(ell[:,None]+1)/ell[:,None]**1.5)
        y_pred = y_pred.numpy()
...
plt.legend()
plt.savefig("chebymlp.pdf", format="pdf", bbox_inches="tight")
...
#plt.legend()
plt.savefig("chebymlperror.pdf", format="pdf", bbox_inches="tight")
...
#plt.axhline(y = 0, color = 'grey', linestyle = '-') 
plt.savefig("chebymlpchi.pdf", format="pdf", bbox_inches="tight")